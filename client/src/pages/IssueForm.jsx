...
export default function IssueForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let record;
      if (isGroup) {
        const res = await axios.post('/api/issues/batch', {
      ...
      if (signatureFile && record?.id) {
        ...
        const sigRes = await axios.post('/api/upload/signature', fd, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setLastSignature(sigRes.data.signature_path || null);
      } else {
  ...
  return (
    <div className="page-wrapper page-issue">
      <div className="page-header">
        <div className="container">
          <div>
            <h1>Выдача спецодежды и СИЗ</h1>
            <div className="page-subtitle">Оперативная выдача, групповая раздача и возвраты</div>
          </div>
        </div>
      </div>
      <div className="container">
          ...
          <form onSubmit={handleSubmit}>
            ...
            {!isGroup && (
              <div className="form-group">
                ...
                />
                {signatureFile && <small style={{ color: 'var(--primary)' }}>Выбран файл: {signatureFile.name}</small>}
              </div>
            )}
            <div className="form-group">
            ...
            <button type="submit" className="btn" disabled={uploading}>
              {uploading ? 'Загрузка...' : (isGroup ? `Выдать всем сотрудникам объекта (${selectedSite ? sites.find(s => s.id == selectedSite)?.name : 'объект не выбран'})` : 'Выдать')}
            </button>
          </form>
          {lastSignature && (
